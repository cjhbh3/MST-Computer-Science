...
def make_move(grid, row, col, character, move):
    ...
    '''
    if (move == 'w'):
        if is_valid_move(grid,row,col,move):
            old_row = list(grid[row])
            new_row = list(grid[row-1])
            indexOf = old_row.index(character)

            old_row[indexOf] = ' '
            new_row[indexOf] = character

            grid[row] = ''.join(old_row)
            grid[row - 1] = ''.join(new_row)
            row = row - 1
    elif (move == 'd'):
        if is_valid_move(grid,row,col,move):
            old_row = list(grid[row])
...
def go_inside_cs_building(next_floor, next_floor_number):
    ...
    next_grid = []
    grid_set = utils.get_campus_map(next_floor)
    next_row = 24
    next_col = 44
    next_set_row = 0
    next_set_col = 2
